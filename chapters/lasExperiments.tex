...
\begin{figure}
\centering
\includestandalone[width=0.6\linewidth]{tikz/alpha}
...
The decoding process depends on the past outputs. Considering only the most probable output value at $i-1$ is not the best approach possible, as the second or third label, might lead to more accurate predictions in the future, therefore a set of the $n$ most likely labels and network states are stored at each decoding time step. Finally the most likely beam hypothesis is selected. Results of a repetition of the previous experiment are shown in figure~\ref{fig:beamSearchExperiment}. The decoding error curve has fewer outliers, which could be an indication that the beam search implementation is working. Additionally the decoding errors to lie under $50\%$ often, but more experiments are required to make sure the improvement stems in fact from beam search. Furthermore a bug the was accidentally introduced during a code restructure. It removed the feature compression in the listener for this experiment, therefore care must be taken when comparing this result with previous ones.

\begin{figure}
\centering
\includestandalone[width=0.49\linewidth]{tikz/beam_search_e40_b39_p05_v10}
\includestandalone[width=0.49\linewidth]{tikz/beam_search_e3540_b39_p05_v10}
\caption{Learning curves using beam search decoding. }
\label{fig:beamSearchExperiment}
\end{figure}
...
TODO: Add experimental results once they become available.
\begin{figure}
\centering
\includestandalone{tikz/las_dropout}
