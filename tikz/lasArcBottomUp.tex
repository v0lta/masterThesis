...
\begin{tikzpicture}[shorten >=1pt,->,draw=black!50, node distance=\layersep,transform shape,rotate=90]  %<-- rotate the NN
  ...
  \tikzstyle{neuron}=[circle,fill=black!25,minimum size=17pt,inner sep=0pt]
    ...
    \foreach \pos [count=\name] in {0,...,7,9} {
        \ifnum \pos=9  %create the x_T input.
          \node[BLSTM rcNeuron, pin=left:
            \rotatebox{-90}{\parbox[t][][r]{8mm}{\centering $x_{T}$}}]
            (I-\name) at (0,-\pos) {};
        \else
          \node[BLSTM rcNeuron, pin=left:
            \rotatebox{-90}{\parbox[t][][r]{8mm}{\centering $x_{\name}$}}]
            (I-\name) at (0,-\pos) {};
        \fi
      }
    ...
    \node[annot,right of=hl,hoz] {Listener: \\ pyramidal BLSTM};

    % draw the listener output nodes
    \node[right of=O-1, node distance=1.0cm, rotate=-90] (h1) {$h_1$};
    \draw[->] (O-1) -- (h1);
    \node[right of=O-2, node distance=1.0cm, rotate=-90] (h2) {$h_2$};
    \draw[->] (O-2) -- (h2);
    \node[right of=O-3, node distance=1.0cm, rotate=-90] (h3) {$h_u$};
    \draw[->] (O-3) -- (h3);
    ...
    \foreach \pos [count=\name] in {0,...,2,4} {
        \ifnum \pos=4.5  %create the x_T input.
          \node[LSTM rcNeuron, pin=left:
            \rotatebox{-90}{\parbox[t][][r]{8mm}{\centering $y_{s-1}$}}]
        ...
        \else
          ...
          \else
            \node[LSTM rcNeuron, pin=left:
              \rotatebox{-90}{\parbox[t][][r]{8mm}{\centering $y_{\name}$}}]
              (SI-\name) at (\layersep*4,-\pos*2-0.5) {};
          \fi
        \fi
      }
      ...
      \node[right of=SO-1, node distance=1.0cm, rotate=-90] (y2) {$y_2$};
      \draw[->] (SO-1) -- (y2);
      \node[right of=SO-2, node distance=1.0cm, rotate=-90] (y3) {$y_3$};
      \draw[->] (SO-2) -- (y3);
      \node[right of=SO-3, node distance=1.0cm, rotate=-90] (y4) {$y_4$};
      \draw[->] (SO-3) -- (y4);
      ...
      \draw[->] (SI-1) -- (A-1);
      \draw[->] (SI-2) -- (A-2);
      \draw[->,dashed] (SI-3) -- (SH-3); %why this arrow??
      \draw[->] (SI-4) -- (A-3);
      \draw[->] (A-1) -- (SH-1);
      \draw[->] (A-2) -- (SH-2);
      \draw[->] (A-3) -- (SH-4);
      ...
      %attend connections.
      \draw[->, bend angle=10, bend left] (h) to (A-1) node[below right, rotate=-90] {$h$};
      \draw[->, bend angle=20, bend right] (h) to (A-2) node[below right, rotate=-90] {$h$};
      \draw[->, bend angle=25, bend right] (h) to (A-3) node[below left, rotate=-90] {$h$};

      %draw the c vector connections.
      \draw[->, bend angle=45, bend left] (A-1) to ($(SI-2) + (0.3,0.25)$) node[above left, rotate=-90] {$c_1$};
      \draw[->, bend angle=45, bend left] (A-2) to ($(SI-3) + (0.3,0.25)$) node[above left, rotate=-90] {$c_2$};
